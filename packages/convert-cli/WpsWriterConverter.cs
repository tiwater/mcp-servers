...
public static class WpsWriterConverter
{
    ...
    public static void ConvertToPdf(string input, string output)
    {
        ...
        if (string.IsNullOrWhiteSpace(python))
        {
            throw new InvalidOperationException(
                "WPS RPC python is required for WPS Writer PDF conversion. Set TIWATER_WPSRPC_PYTHON or LUCID_WPSRPC_PYTHON.");
        }
        ...
        var helperPath = Path.Combine(tempRoot, "docx_to_pdf_wps.py");
        ...
        try
        {
            ...
            if (!process.WaitForExit(TimeSpan.FromMinutes(2)))
            {
                ...
                throw new TimeoutException("WPS Writer PDF conversion timed out after 120 seconds.");
            }
...
try:
    ...
    try:
        hr = document.ExportAsFixedFormat(
            output_path,
            wpsapi.WdExportFormat.wdExportFormatPDF,
...
if not os.path.isfile(output_path):
    raise SystemExit(f"WPS Writer did not create output: {output_path}")
""";
}
