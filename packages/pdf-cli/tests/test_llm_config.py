...
class ResolveLlmConfigTest(unittest.TestCase):
    ...
    def test_rejects_non_supen_credentials(self):
        with patch.dict(os.environ, {"OPENROUTER_API_KEY": "openrouter-token"}, clear=True):
            with self.assertRaisesRegex(RuntimeError, "SUPEN_LLM_TOKEN"):
                _resolve_llm_config()

    def test_rejects_explicit_credentials(self):
        env = {
            "SUPEN_LLM_TOKEN": "gateway-token",
            "SUPEN_LLM_GATEWAY_URL": "http://127.0.0.1:2755/api/llm/v1",
        }
        ...
        with patch.dict(os.environ, env, clear=True):
            with self.assertRaisesRegex(RuntimeError, "per-invocation Supen environment"):
                _resolve_llm_config(api_key="explicit-token", base_url="https://llm.example/v1")
