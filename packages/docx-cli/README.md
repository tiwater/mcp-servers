...
`replaceTableCellRichText` accepts `richText` segments with `text`, optional `color`, `underline`, `bold`, `italic`, `fontName`, and `verticalAlignment` (`baseline`, `superscript`, or `subscript`). Table inspection returns the same run-level vertical alignment so callers can preserve formulas without flattening their script semantics.
...
`applyTocStylePolicy` updates every built-in TOC paragraph style as one Office object family. `italic` is explicit, and `indentCharactersPerLevel` gives TOC level 1 zero indentation with each deeper level adding that many characters. Documents without built-in TOC styles fail closed.
`validate-toc-style-policy <input.docx> <italic> <indentCharactersPerLevel>` independently reads those styles back and fails on a missing style family, indentation drift, or italic drift.
