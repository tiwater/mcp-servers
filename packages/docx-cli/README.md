...
Image drawing records include the OpenXML drawing id, the direct owning paragraph or table-cell anchor, complete ordered section bindings, paragraph/table/row/cell identities, inline or anchored placement metadata, the image relationship and package part, and a SHA-256 hash computed from the actual image bytes. A singular `SectionId` is emitted only when exactly one section owns the drawing; `SectionIds` is authoritative for linked header/footer parts shared by multiple sections. Anchored placement distinguishes alignment from offset positioning and records simple-position coordinates, relative height, layout flags, anchor distances, exact effect extents, and wrap mode/details. Tight and through wrapping includes the polygon's edited flag, start point, and ordered line points. Inspection fails closed when drawing ids are duplicated, an embedded image relationship is missing or does not target an image part, or required tight/through polygon evidence is malformed.

Paragraph run evidence follows all valid nested run containers in document order, including hyperlinks and run-level content controls, so formatted run evidence corresponds to the paragraph's descendant text rather than only direct child runs.
...
Exports body table formatting from each property's owning OpenXML element: table width/type/layout, row height/rule and split behavior, cell width/type/no-wrap/alignment/shading, grid spans and vertical merges, paragraph style/alignment, and per-run style, font, size, color, bold, italic, underline, and text-fill details. Null-valued optional properties are omitted from JSON output; absent on/off properties are reported as `false`. Use this for template-fidelity validation where table shape and direct formatting matter.
