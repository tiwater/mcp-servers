...
- `setPageSetup` with required `sheet` and at least one of `fitToPagesWide`, `fitToPagesTall`, or `orientation` (`portrait` or `landscape`); an omitted fit dimension remains unconstrained
...
```json
{
  "operations": [
    ...
    { "type": "setPageSetup", "sheet": "Sheet1", "fitToPagesWide": 1, "orientation": "landscape" },
