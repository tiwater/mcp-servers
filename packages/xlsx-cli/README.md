...
- `setPageSetup` with required `sheet` and at least one of `fitToPagesWide`, `fitToPagesTall`, `orientation` (`portrait` or `landscape`), or `paperSize` (`letter`, `legal`, `a3`, or `a4`); an omitted fit dimension remains unconstrained
...
```json
{
  "operations": [
    ...
    { "type": "setPageSetup", "sheet": "Sheet1", "fitToPagesWide": 1, "orientation": "landscape", "paperSize": "a3" },
