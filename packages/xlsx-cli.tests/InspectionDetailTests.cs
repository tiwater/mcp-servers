...
public class InspectionDetailTests
{
    ...
    public async Task Published_inspection_evidence_attests_package_semver_for_producer_validator_and_nested_evidence()
    {
        ...
        Assert.Equal("0.2.25", produced.RootElement.GetProperty("provider").GetProperty("toolVersion").GetString());
        Assert.Equal("0.2.25", validated.RootElement.GetProperty("validator").GetProperty("toolVersion").GetString());
        Assert.Equal("0.2.25", produced.RootElement.GetProperty("observations")[0].GetProperty("value").GetProperty("evidence").GetProperty("toolVersion").GetString());
