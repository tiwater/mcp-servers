...
2. a new `docx_plan` command in `docx-cli`
3. Office MCP exposure for `docx_plan`
...
- `skills/stability-report/SKILL.md`
  - Update the skill flow to require `docx_plan` before `docx_edit`.
...
## Task 3: Add `docx_plan` Models and Planner
...
## Task 5: Expose `docx_plan` in Office MCP
...
- [ ] Update `servers/office/README.md` to list `docx_plan`.

## Task 6: Document `docx_plan`

- [ ] Update `packages/docx-cli/README.md`.

Document:

- when to use `docx_plan`
...
- `docx_plan` produces structured plan items
...
2. `docx_plan` classifies direct, table-scope, and narrative comments on synthetic fixtures
3. Office MCP exposes `docx_plan`
4. `stability-report` uses `docx_plan` explicitly
